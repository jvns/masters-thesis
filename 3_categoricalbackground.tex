In the introduction we explained that models of TQC can be presented as modular
tensor categories.  The basic idea is that a modular tensor category is a
semisimple category where anyon types correspond to isomorphism classes of
simple objects. Fusing two anyons corresponds to taking the tensor product of
two objects, and the meaning of an equation like

\begin{equation}
V_1 \otimes V_2 \simeq V_3 \oplus V_4
\end{equation}

for simple objects $V_1, V_2, V_3, V_4$ is that fusing $V_1$ with $V_2$ results in
either an anyon of type $V_3$ or an anyon of type $V_4$ (or a superposition, of
course).

In categorical terms, a modular tensor category needs to be:

\begin{itemize}
    \item semisimple (fusing anyons results in an anyon)
    \item monoidal (so that we can have tensor products of several anyons)
    \item rigid (has duals)
    \item braided (so that anyons can be exchanged: this is how computation occurs)
    \item ribbon (has a notion of a trace)
\end{itemize}

A category with all these properties that satisfies one additional condition is
called a modular tensor category. We will explain this condition and discuss
modular tensor categories in more detail in section \ref{section:MTCs}. In
this chapter, we give abstract definitions
for all the properties a MTC needs to satisfy. We also show that the category
...
braided monoidal category: any computation done a set of anyons corresponds to
