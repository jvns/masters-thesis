Throughout we will be working over a field $k$ of characteristic 0.
...
\begin{theorem}
    $\mathbf{Z}_2(\mathcal{C})$ is trivial if and only if the matrix $s$ is invertible.  \todo{find a citation for this}
\end{theorem}

\begin{example}
    An elementary class of examples of modular tensor categories can be
    obtained as follows. This is a simple example of Drinfeld's quantum double
    construction which can be applied to any finite dimensional Hopf algebra.

    \todo{explain the quantum double construction later?}
    ...
    The quantum double $D(G)$ of $k(G)$ can be described as follows: 
    
    $D(G)$ is the Hopf algebra with vector space $F(G) \otimes_k k[G]$ and 
